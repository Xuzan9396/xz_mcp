...
// CreateTable 创建表
func CreateTable(createSQL string) (*ExecResult, error) {
	if !IsConnected() {
		return nil, fmt.Errorf("database not connected")
	}

	// DDL操作不返回affected rows，所以我们需要直接检查错误
...
// AlterTable 修改表结构
func AlterTable(alterSQL string) (*ExecResult, error) {
	if !IsConnected() {
		return nil, fmt.Errorf("database not connected")
	}
